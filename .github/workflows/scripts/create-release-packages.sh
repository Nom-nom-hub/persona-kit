...
cp -r patterns/ "$TEMP_DIR/" 2>/dev/null || echo "No patterns directory found, skipping"
cp -r personas/ "$TEMP_DIR/" 2>/dev/null || echo "No personas directory found, skipping"
cp -r workflows/ "$TEMP_DIR/" 2>/dev/null || echo "No workflows directory found, skipping"

# Create .persona-kit directory with basic structure
...
for assistant in "${AI_ASSISTANTS[@]}"; do
    for script_type in "${SCRIPT_TYPES[@]}"; do
        ...
        # Copy the template files to this package
        cp -r "$TEMP_DIR"/templates "$package_dir/"
        
        # If patterns directory exists, copy it
        if [ -d "$TEMP_DIR/patterns" ]; then
            cp -r "$TEMP_DIR/patterns" "$package_dir/"
        fi
        
        # If personas directory exists, copy it
        if [ -d "$TEMP_DIR/personas" ]; then
            cp -r "$TEMP_DIR/personas" "$package_dir/"
        fi
        
        # If workflows directory exists, copy it
        if [ -d "$TEMP_DIR/workflows" ]; then
            cp -r "$TEMP_DIR/workflows" "$package_dir/"
        fi
...
POWERSHELL_EOF
        ...
        # Create the slash command files based on templates
        cp "$TEMP_DIR/templates/constitution-cmd-template.md" "$package_dir/constitution-cmd.md" 2>/dev/null || echo "constitution-cmd-template.md not found"
        cp "$TEMP_DIR/templates/personas-cmd-template.md" "$package_dir/personas-cmd.md" 2>/dev/null || echo "persons-cmd-template.md not found"
        cp "$TEMP_DIR/templates/patterns-cmd-template.md" "$package_dir/patterns-cmd.md" 2>/dev/null || echo "patterns-cmd-template.md not found"
        cp "$TEMP_DIR/templates/workflows-cmd-template.md" "$package_dir/workflows-cmd.md" 2>/dev/null || echo "workflows-cmd-template.md not found"
